...
void* positionhread(void* arg);
int stop;
int moving; //moving =0 if beginner, 1 if finisher and 2 if waiting
//Ball
void* ballthread(void* arg);
pthread_mutex_t mutex_ball = PTHREAD_MUTEX_INITIALIZER; 
pthread_cond_t condition_ball = PTHREAD_COND_INITIALIZER;
	...
	while(1){
		if (moving == 0){
			...
			*((uint16_t *) string) = msgId++;
			...
			printf("I'm not supposed to send my coordinates");		
		}
...
void* positionthread()	/*thread calculating the position of the robot in real time*/
{
	while(1){
		if (case_pos == 0) {
			...
			mes_sonar(sn_sonar, value);
			pthread_mutex_lock(&mutex_y_robot);
			y_robot = (int16_t) (12 - (value_sonar / 10));
			pthread_mutex_unlock(&mutex_y_robot);
		}
		else if (case_pos == 4) {
...
void* finisher(){
	...
	if (position==3){ //Finisher of the right side
		...
		*((uint16_t *) string) = msgId++;
		...
		avancer_dst(sn1, sn2, sn_gyro, motor, value, gyro_val, gyro_abs, state, 200);
		tourner_droite(sn1, sn2, sn_gyro, motor, value, gyro_val, gyro_abs, state);
		bouger_sonar(sn1, sn2, sn_gyro, sn_sonar, motor, value, gyro_val, gyro_abs, state, 230);
...
int main() {
	...
	if( status == 0 ) {
		...
		if (rank == 0){
			...
			pthread_mutex_unlock (&mutex_moving); /* Unlocking the mutex */ //we are the beginner 
		}
		else{
			if (side == 0){
